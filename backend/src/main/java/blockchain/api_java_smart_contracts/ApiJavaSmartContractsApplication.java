    ...
    public CommandLineRunner commandLineRunner() {
        return args -> {
            ...
            try {
                ...
                if (currentContractAddress == null || currentContractAddress.trim().isEmpty()
                        || "0x2835ccb5F1D4d122Ebfa963F0616BEe5af1090FE".equalsIgnoreCase(currentContractAddress)) {
                    ...
                    currentContractAddress = blockchainService.deployContractAndReturnAddress().join(); // .join()
                                                                                                        // espera a
                                                                                                        // conclusão
                    logger.info("Contrato Voting deployado e configurado no endereço: {}", currentContractAddress);
                } else {
                    logger.info("Contrato Voting encontrado no endereço configurado: {}. Carregando...",
                ...
                BigInteger totalVotos = blockchainService.getTotalVotos().get(); // .get() para esperar o resultado do
                                                                                 // CompletableFuture
                ...
                logger.error("Erro durante a inicialização/interação com o contrato na blockchain: {}", e.getMessage(),
                        e);
                // Para depuração, também imprima o stack trace completo
                logger.error("Stack trace completo do erro:", e);
            } catch (IllegalStateException e) {
                logger.error("Erro de estado ilegal: Contrato Voting não foi inicializado corretamente. {}",
                        e.getMessage(), e);
